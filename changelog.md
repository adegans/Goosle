...
## April 6, 2026
The end, GooseTOR replaces Goosle starting today. Goosle is dead! Finished! Done! It will no longer get any updates.  
GooseTOR is the new and improved Torrent Meta Search engine. Inspired by Goosle but re-done from the ground up.

More search engines, better filtering options, faster scraping.  
An all new theme and front-end. No more broken accounts and shitty patch-jobs for code, and no more bullshit features that barely worked.

GooseTOR does one thing and one thing only, and that is search for content so you can enjoy the internet.

Grab your copy here: [GooseTOR on Github](https://github.com/adegans/GooseTOR).

### March 16, 2026 (2.0b61)
- NOTICE: config.default.php has changed, update your config.php!!
- new: Management page to manage tokens, timeouts and other features
- new: Basic accounts system (See config.default.php to enable it)
- new: Option to make accounts optional or required
- new: Invite link for new accounts
- new: Add your own engines by placing new ones in the engines/ folder
- new: Template example engine so you can make your own
- new: IMDb link for supported movie results
- change: Added YTS and EZTV highlights to cron job
- change: Accounts replace the hash_auth feature
- change: Setting show_nsfw_magnets merged into Safe Search setting
- change: Removed safe:on and safe:off as overrides
- change: Share option now shares the Magnet Link instead of a highlighted result
- fix: Better result matching when urls are used for internal rank
- fix: Better result matching by age for video and news results
- fix: CSS tweaks to improve readability for lists
- fix: Incorrect pixel definition for magnet info popups
- fix: Authorization error not centered and text too small
- fix: Better check for keyword searches
- remove: Web/text search
- remove: Image search
- remove: News search
