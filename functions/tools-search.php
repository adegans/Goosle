...
// Results pagination
--------------------------------------*/
function search_pagination($search, $baseurl, $results_per_page, $number_of_results) {
	...
	if($search->page > 1) {
		$prev = $search->page - 1;
		$pagination .= "<a href=\"".$baseurl."results.php?q=".$search->query_urlsafe."&p=".$prev."\" title=\"Previous page\"><span class=\"arrow-left\"></span></a> ";
	}

	for($page = 1; $page <= $number_of_pages; $page++) {
		$class = ($search->page == $page) ? "current" : "";
		$pagination .= "<a href=\"".$baseurl."results.php?q=".$search->query_urlsafe."&p=".$page."\" class=\"".$class."\" title=\"To page ".$page."\">".$page."</a> ";
	}

	if($search->page < $number_of_pages) {
		$next = $search->page + 1;
		$pagination .= "<a href=\"".$baseurl."results.php?q=".$search->query_urlsafe."&p=".$next."\" title=\"Next page\"><span class=\"arrow-right\"></span></a> ";
	}
...
--------------------------------------*/
function highlight_popup($opts_pixel, $highlight) {
	$meta = $magnet_meta = array();

	$search_query = urlencode($highlight['title']." ".$highlight['year']);
	$thumb = (!empty($highlight['thumbnail'])) ? $highlight['thumbnail'] : $opts_pixel;
	...
	$output .= "		<br /><br /><a href=\"https://www.imdb.com/title/".$highlight['imdb_id']."/\" target=\"_blank\" title=\"View on imdb.com\">View on imdb.com</a> &bull; <a href=\"./results.php?q=".$search_query."\" title=\"Search on Goosle Magnet Search! For new additions results may be limited.\">Find more Magnet links</a>";
	$output .= "</p>";
	unset($meta);
